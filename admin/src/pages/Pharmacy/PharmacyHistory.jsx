...
import { History, Search, User, Stethoscope, Pill } from 'lucide-react'
  ...
  return (
    <div className="curalink-fade-in" style={{ minHeight: '100vh', background: 'linear-gradient(180deg, #F8FAFC 0%, #EEF6FF 100%)', padding: '32px 24px 48px' }}>
      <div style={{ maxWidth: 1000, margin: '0 auto' }}>
        <PageHero icon={History} title="Dispensing History" description="Every dispensing transaction this pharmacy has performed — one entry per action, never overwritten." />
              ...
              <div key={i} style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: 18, padding: '16px 20px', boxShadow: '0 4px 16px rgba(15,23,42,0.04)' }}>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 10 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
                    ...
                    <span style={{ display: 'flex', alignItems: 'center', gap: 5, fontSize: 12, color: '#475569' }}>
                      <Stethoscope size={13} color="#94A3B8" /> {item.doctor?.name}{item.doctor?.speciality ? ` (${item.doctor.speciality})` : ''}
                    </span>
                  </div>
                  <span style={{ fontSize: 10.5, fontWeight: 700, padding: '5px 12px', borderRadius: 999, background: '#F0FDFA', color: '#0D9488', display: 'flex', alignItems: 'center', gap: 5 }}>
                    <Pill size={12} /> {item.quantityDispensed} × {item.medicineName}
                  </span>
                </div>
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 16, marginTop: 10, fontSize: 11.5, color: '#94A3B8' }}>
                  ...
                  <span>Dispensed: {formatDateTime(item.dispensedAt)}</span>
